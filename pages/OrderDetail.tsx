        ...
        <div className="min-h-screen bg-[#fafafa] pb-20">
            <div className="max-w-[1200px] mx-auto px-4 md:px-8 pt-8">
                ...
                <div className="text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-6">
                    ...
                    <Link to="/profile/orders" className="hover:text-black transition-colors border-b border-[#c8102e] text-black pb-0.5">Đơn hàng của tôi</Link>
                    <span className="mx-2">›</span>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                    <div>
                        <h1 className="text-2xl font-black uppercase text-gray-900 tracking-tighter">
                            CHI TIẾT ĐƠN HÀNG <span className="text-[#c8102e]">#{order.orderNumber}</span>
                        </h1>
                ...
                {order.status !== 'CANCELLED' && (
                    <div className="bg-white border border-gray-200 p-8 mb-8 relative">
                        ...
                        <div className="absolute top-1/2 left-16 h-0.5 bg-[#c8102e] -translate-y-1/2 hidden md:block transition-all duration-500" 
                        ...
                        <div className="flex flex-col md:flex-row justify-between relative z-10 gap-8 md:gap-0">
                            ...
                            <div className="flex flex-col items-center bg-white px-4">
                                <div className={`w-12 h-12 rounded-lg flex items-center justify-center mb-3 ${currentStep >= 1 ? 'bg-[#c8102e] text-white' : 'bg-gray-100 text-gray-400'}`}>
                                    <Package size={24} />
                                </div>
                                <div className="text-center">
                                    <p className={`text-xs font-bold ${currentStep >= 1 ? 'text-[#c8102e]' : 'text-gray-400'}`}>{order.orderDate}</p>
                                    <p className="text-sm font-bold text-gray-800 mt-1">Đã đặt hàng</p>
                                </div>
                            </div>
                            {/* Step 2 */}
                            <div className="flex flex-col items-center bg-white px-4">
                                <div className={`w-12 h-12 rounded-lg flex items-center justify-center mb-3 ${currentStep >= 2 ? 'bg-[#c8102e] text-white' : 'bg-gray-100 text-gray-400'}`}>
                                    <Banknote size={24} />
                                </div>
                                <div className="text-center">
                                    <p className={`text-xs font-bold ${currentStep >= 2 ? 'text-[#c8102e]' : 'text-gray-400'}`}>{currentStep >= 2 ? 'Hoàn tất' : 'Đang chờ'}</p>
                                    <p className="text-sm font-bold text-gray-800 mt-1">Đã thanh toán</p>
                                </div>
                            </div>
                            {/* Step 3 */}
                            <div className="flex flex-col items-center bg-white px-4">
                                <div className={`w-12 h-12 rounded-lg flex items-center justify-center mb-3 ${currentStep >= 3 ? 'bg-[#c8102e] text-white' : 'bg-gray-100 text-gray-400'}`}>
                                    <Truck size={24} />
                                </div>
                                <div className="text-center">
                                    <p className={`text-xs font-bold ${currentStep >= 3 ? 'text-[#c8102e]' : 'text-gray-400'}`}>{currentStep >= 3 ? 'Đang giao' : 'Đang chờ'}</p>
                                    <p className="text-sm font-bold text-gray-800 mt-1">Đang vận chuyển</p>
                                </div>
                            </div>
                            {/* Step 4 */}
                            <div className="flex flex-col items-center bg-white px-4">
                                <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 ${currentStep >= 4 ? 'bg-[#c8102e] text-white' : 'bg-gray-100 text-gray-400'}`}>
                                    <CheckCircle size={24} />
                                </div>
                                <div className="text-center">
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="flex-1 space-y-6">
                        ...
                        <div className="bg-white border border-gray-200 p-8">
                            <h2 className="text-[15px] font-bold uppercase tracking-widest text-gray-800 mb-6">
                                        ...
                                        <div key={item.id} className="py-6 first:pt-0 last:pb-0 flex gap-6">
                                            ...
                                            <div className="flex-1 flex flex-col justify-between">
                                                <div className="flex justify-between items-start gap-4">
                                                    <div>
                                                        <h3 className="text-[15px] font-bold text-gray-800 mb-1 leading-snug">{productName}</h3>
                                                        <p className="text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1">
                                                            PHÂN LOẠI: {colorValue} / SIZE {sizeValue}
                                                        </p>
                                                        <p className="text-sm font-medium text-gray-800">Số lượng: {item.quantity.toString().padStart(2, '0')}</p>
                                                    </div>
                                                    <div className="text-[15px] font-bold text-[#c8102e] whitespace-nowrap">
                                                ...
                                                <div className="flex gap-3 mt-4">
                                                    <button
                                                        ...
                                                        className={`px-6 py-2 text-[11px] font-bold uppercase tracking-widest transition-colors ${canReview ? "bg-[#c8102e] text-white hover:bg-red-800" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}
                                                    >
                                                        {item.reviewed ? "ĐÃ ĐÁNH GIÁ" : isExpired ? "HẾT HẠN" : "ĐÁNH GIÁ"}
                                                    </button>
                                                    <button onClick={() => navigate(`/product/${productId}`)} className="px-6 py-2 text-[11px] font-bold uppercase tracking-widest border border-black text-black bg-white hover:bg-black hover:text-white transition-colors">
                                                        MUA LẠI
                                                    </button>
                                                </div>
                                            </div>
                        ...
                        {order.status === 'SHIPPED' && (
                            <div className="bg-white border border-gray-200 p-6 flex items-start gap-4">
                                <div className="text-[#c8102e] mt-1"><Truck size={24} /></div>
                                <div>
                                    <p className="font-bold text-gray-900 text-[15px]">Đơn hàng đang được vận chuyển.</p>
                    ...
                    <div className="w-full lg:w-80 shrink-0 space-y-6">
                        ...
                        <div className="bg-white border border-gray-200 p-6">
                            ...
                            <p className="font-bold text-[15px] text-gray-900 mb-1">{order.shippingAddress?.fullName || 'Khách hàng'}</p>
                        ...
                        {/* Summary */}
                        <div className="bg-white border border-gray-200 border-t-4 border-t-[#c8102e] p-6">
                            ...
                            <div className="space-y-4 text-sm font-medium mb-6">
                                <div className="flex justify-between text-gray-600">
                                    <span>Tạm tính</span>
                                    <span className="text-gray-900">{order.totalPrice.toLocaleString()}đ</span>
                                </div>
                                <div className="flex justify-between text-gray-600">
                            ...
                            <div className="border-t border-gray-100 pt-6 flex justify-between items-end">
                                ...
                                <span className="text-2xl font-black text-[#c8102e]">{order.totalPrice.toLocaleString()}đ</span>
                            </div>
                        </div>
                    </div>
                </div>
