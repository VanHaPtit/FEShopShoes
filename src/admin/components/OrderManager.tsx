        ...
        <div className="bg-slate-50 min-h-[85vh] p-4 text-[13px] font-sans text-slate-700 rounded-lg">
            ...
            <div className="bg-slate-100/80 p-5 rounded-lg mb-6 border border-slate-200">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                    ...
                    <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="border border-slate-300 rounded-md px-3 py-2.5 outline-none focus:border-blue-400 bg-white text-slate-600 font-medium">
                        <option value="">- Tất cả trạng thái đơn hàng -</option>
                        {Object.values(OrderStatus).map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                    <input type="date" value={dateFilter} onChange={e => setDateFilter(e.target.value)} title="Ngày tạo đơn" className="border border-slate-300 rounded-md px-3 py-2.5 outline-none focus:border-blue-400 bg-white text-slate-600 font-medium" />
                    <select value={paymentFilter} onChange={e => setPaymentFilter(e.target.value)} className="border border-slate-300 rounded-md px-3 py-2.5 outline-none focus:border-blue-400 bg-white text-slate-600 font-medium">
