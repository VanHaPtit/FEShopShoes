...
const CategoryManager: React.FC<CategoryManagerProps> = ({ categories, onSave, onDelete }) => {
    ...
    return (
        <div className="space-y-8 bg-[#fafafa] p-6 md:p-8 min-h-screen font-sans">
            {/* Header Section */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-[22px] font-black text-gray-900 tracking-tight">Category Management</h2>
                    <p className="text-[13px] text-gray-500 mt-1">Organize and manage your product store hierarchical structure.</p>
                </div>
                <button
                    onClick={handleAddNew}
                    className="bg-[#c8102e] text-white px-6 py-2.5 text-[12px] font-bold uppercase tracking-widest hover:bg-red-800 transition-colors flex items-center justify-center gap-2 w-full sm:w-auto"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {categories.map((cat, index) => {
                    ...
                    if (isSale) {
                        return (
                            <div key={cat.id} className="bg-[#c8102e] text-white flex flex-col group shadow-[0_2px_10px_rgba(0,0,0,0.04)]">
                                {/* Image Area */}
                                ...
                                <div className="p-5 flex flex-col flex-grow justify-between gap-4 bg-[#c8102e]">
                                    <div>
                                    ...
                                    <div className="flex items-center gap-2 pt-2">
                                        <button
                                            ...
                                            className="flex-1 flex items-center justify-center py-2.5 text-[11px] font-bold uppercase tracking-widest text-[#c8102e] bg-white transition-colors"
                                        >
                                            MANAGE OFFERS
                                        </button>
                                        <button
                                            onClick={() => cat.id && onDelete(cat.id)}
                                            className="w-10 h-10 flex items-center justify-center border border-red-500 hover:bg-red-800 transition-colors flex-shrink-0"
                                        >
                    ...
                    return (
                        <div key={cat.id} className="bg-white border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)] hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col group">
                            {/* Image Area */}
                            <div className="h-56 bg-gray-50 relative overflow-hidden">
                                {index === 0 && !isSale && (
                                    <div className="absolute top-3 right-3 z-10 bg-[#c8102e] text-white text-[9px] font-bold px-2 py-1 uppercase tracking-wider">
                                        TRENDING
                                    </div>
                                )}
                                {cat.image ? (
                            ...
                            <div className="p-5 flex flex-col flex-grow justify-between gap-4">
                                <div>
                                    <h3 className="font-black text-gray-900 text-[16px] uppercase tracking-wide line-clamp-1" title={cat.name}>
                                        {cat.name}
                                    </h3>
                                ...
                                <div className="flex items-center gap-2 pt-3">
                                    <button
                                        onClick={() => handleEdit(cat)}
                                        className="flex-1 flex items-center justify-center py-2.5 text-[11px] font-bold uppercase tracking-widest text-black border border-black hover:bg-black hover:text-white transition-colors"
                                    >
                                        EDIT
                                    </button>
                                    <button
                                        onClick={() => cat.id && onDelete(cat.id)}
                                        className="w-10 h-10 flex items-center justify-center border border-gray-200 text-gray-400 hover:text-black hover:border-black transition-colors flex-shrink-0"
                                    >
                ...
                <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
                    ...
                    >
                        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-white">
                            <h3 className="text-[16px] font-black uppercase tracking-wide text-gray-900">
                                {editingCat ? 'UPDATE CATEGORY' : 'NEW CATEGORY'}
                            </h3>
                            <button type="button" onClick={() => setIsFormOpen(false)} className="text-gray-400 hover:text-black transition-colors">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            <div>
                                <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-2">
                                    CATEGORY NAME <span className="text-[#c8102e]">*</span>
                                </label>
                                <input
                                    type="text"
                                    value={name}
                            ...
                            <div>
                                ...
                                <div
                                    ...
                                    className={`relative mt-1 flex flex-col items-center justify-center p-6 border border-dashed cursor-pointer transition-all duration-200 min-h-[160px]
                                        ${preview ? 'border-gray-200 bg-gray-50' : 'border-gray-300 hover:border-black hover:bg-gray-50'}`}
                                >
                                    {preview ? (
                        ...
                        <div className="px-6 py-5 bg-gray-50 border-t border-gray-100 flex gap-3 sm:justify-end">
                            <button
                                type="button"
                                onClick={() => setIsFormOpen(false)}
                                className="w-full sm:w-auto px-6 py-2.5 bg-white border border-gray-200 text-[11px] font-bold uppercase tracking-widest text-black hover:bg-gray-100 transition-colors"
                            >
                                CANCEL
                            </button>
                            <button
                                type="submit"
                                className="w-full sm:w-auto px-6 py-2.5 bg-[#c8102e] text-white text-[11px] font-bold uppercase tracking-widest hover:bg-red-800 transition-colors flex items-center justify-center gap-2"
                            >
