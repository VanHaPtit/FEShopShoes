        ...
        <div className="bg-slate-50 min-h-[85vh] p-4 text-[13px] font-sans text-slate-700 rounded-lg">
            ...
            <div className="bg-slate-100/80 p-5 rounded-lg mb-6 border border-slate-200">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                    <input type="text" placeholder="Tìm theo tên khách, mobile, hoặc mã đơn..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="border border-slate-300 rounded-md px-3 py-2.5 outline-none focus:border-blue-400 placeholder:text-slate-400" />
                    <select className="border border-slate-300 rounded-md px-3 py-2.5 outline-none focus:border-blue-400 bg-white text-slate-600 font-medium">
                        <option value="">- Tất cả trạng thái đơn hàng -</option>
                        {Object.values(OrderStatus).map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                    <input type="date" title="Ngày tạo đơn" className="border border-slate-300 rounded-md px-3 py-2.5 outline-none focus:border-blue-400 bg-white text-slate-600 font-medium" />
                    <select className="border border-slate-300 rounded-md px-3 py-2.5 outline-none focus:border-blue-400 bg-white text-slate-600 font-medium">
            ...
            <div className="flex justify-between items-end mb-4 px-1">
                ...
                <div className="flex gap-2.5">
                    <button className="bg-[#8ab8e6] hover:bg-blue-400 text-white px-4 py-2 rounded-md flex items-center gap-2 shadow-sm font-medium transition-colors">
                        Hành động <span className="text-[10px]">▼</span>
                    </button>
                    <button className="bg-[#d9534f] hover:bg-red-600 text-white px-4 py-2 rounded-md shadow-sm font-medium transition-colors">Còn nợ tiền</button>
                    <button className="bg-[#f0ad4e] hover:bg-amber-500 text-white px-4 py-2 rounded-md shadow-sm font-medium transition-colors">Cần xử lý</button>
                    <button className="bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 px-4 py-2 rounded-md flex items-center gap-2 shadow-sm font-medium transition-colors">
                        ⚙️ Chọn cột
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm overflow-x-auto border border-slate-200">
                <table className="w-full text-left border-collapse min-w-max">
                    <thead className="bg-[#f9fafb] text-slate-700 border-b border-slate-200 text-[12px] font-bold">
                        <tr>
                            <th className="p-3 w-10 text-center"><input type="checkbox" className="w-4 h-4 rounded border-slate-300" /></th>
                        ...
                        {filteredOrders.map((order) => {
                            ...
                            return (
                                <tr key={order.id} className={`${isEditing ? 'bg-blue-50/40' : 'hover:bg-slate-50'} transition-colors group`}>
                                    <td className="p-3 text-center"><input type="checkbox" className="w-4 h-4 rounded border-slate-300" /></td>
                                    ...
                                    <td className="p-3 text-center">
                                        {isEditing ? (
                                            ...
                                            <span className={`px-2.5 py-1.5 rounded text-white text-[10.5px] font-bold whitespace-nowrap inline-block min-w-[70px] text-center shadow-sm ${
                                                order.status === OrderStatus.DELIVERED ? 'bg-[#28a745]' : 
                                    ...
                                    <td className="p-3 text-center">
                                        <span className={`px-2 py-1.5 rounded text-white text-[10.5px] font-bold whitespace-nowrap inline-block shadow-sm ${
                                            (order as any).payment?.paymentMethod === 'PAYPAL' ? 'bg-[#003087]' : 'bg-[#1a5b82]'
                                        }`}>
                            ...
                            <div className="space-y-3">
                                ...
                                {selectedOrder.items.map((item, idx) => (
                                    <div key={idx} className="flex items-center gap-4 p-3 bg-white border border-slate-200 rounded-lg">
                                        <img src={item.image} className="w-12 h-12 object-cover rounded border border-slate-100 shadow-sm" alt="" />
