  ...
  return (
    <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col h-full">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h3 className="text-lg font-black uppercase tracking-widest text-gray-900">Sản phẩm bán chạy</h3>
      ...
      <div className="space-y-4 flex-grow">
        {topProducts.length === 0 ? (
          ...
          topProducts.map((product, index) => (
            <div key={index} className="flex items-center gap-4 p-3 rounded-2xl hover:bg-gray-50 transition-colors group">
              ...
              <div className="flex-grow min-w-0">
                <h4 className="text-sm font-bold text-gray-900 truncate group-hover:text-black">{product.name}</h4>
                <p className="text-xs text-gray-500 font-medium">{product.totalSold} đôi đã bán</p>
              </div>
              <div className="text-right">
                <p className="text-sm font-black text-gray-900">{formatCurrency(product.totalRevenue)}</p>
